...
fn collect_composite_formatter_string_literals(
    layouts: &BTreeMap<String, StructLayout>,
...
fn collect_enum_formatter_string_literals(
    enum_layouts: &BTreeMap<String, EnumLayout>,
    strings: &mut BTreeMap<String, StringLiteral>,
    next_id: &mut usize,
) {
...
fn enum_formatter_label(enum_name: &str, variant: &str) -> String {
    format!("{enum_name}.{variant}")
}

fn struct_formatter_prefix(struct_name: &str) -> String {
    format!("{struct_name} {{ ")
}

fn struct_field_formatter_label(field_name: &str) -> String {
    format!("{field_name}: ")
}
...
",
        ...
        assert!(
            rendered
                .contains("define private ptr @ax_string_replace(ptr %text, ptr %from, ptr %to)")
        );
        assert!(
            rendered
                .contains("define private ptr @ax_string_replace_empty_from(ptr %text, ptr %to)")
        );
