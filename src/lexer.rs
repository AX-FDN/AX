...
mod tests {
    ...
    fn tokenizes_basic_items() {
        let source = SourceFile::anonymous(
            "pub const EXIT_OK: i32 = 0; fn main() -> i32 { let mut value: i32 = 1; }",
        );
        ...
        assert!(kinds.contains(&TokenKind::PubKw));
    ...
    #[test]
    fn tokenizes_continue_keyword() {
        let source =
            SourceFile::anonymous("fn main() -> i32 { while (true) { continue; } return 0; }");
        let output = tokenize(&source);
        let kinds = output
            .tokens
            .iter()
            ...
            .collect::<Vec<_>>();
        assert!(output.diagnostics.is_empty());
        assert!(kinds.contains(&TokenKind::ContinueKw));
    }
