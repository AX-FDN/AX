...
- 它不是第二套设计基线；项目路线与最终决策仍以 [`执行路线.md`](./执行路线.md) 为准。
...
- `main`
- `string_list` 类型与 `string_list_new / string_list_push / string_list_join / string_list_get`
- `std.collections` 当前提供 `string_list_empty / string_list_append / string_list_count / string_list_join_with / string_list_at / string_list_contains / string_list_index_of`，作为 `string_list_*` builtin 的官方源码级包装
- 只读切片类型、切片表达式与切片索引读取
- 局部变量
- 变量赋值
- 结构体字段赋值
...
- Builtin helpers are println(...), string_len(text), string_list_new(), string_list_push(list, value), string_list_join(list, separator), string_list_get(list, index), len(value), and to_string(value). `std.collections` wraps the string-list helpers for project code that imports `std/`.
