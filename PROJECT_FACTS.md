...
| build | AOT/native backend 正处在 LLVM v0 能力包扩展阶段；`build-manifest.json` schema v9 与 `context evidence` 已暴露结构化 `aot_readiness` 和 blocker-level AI 建议，`axc build --json` 会打印同一个 manifest 对象，并且 `axc build` 可为单文件 MIR 子集生成 `generated/main.ll` LLVM IR v0；当前 AOT parity 已覆盖 18 个 core/stdout/string/array-read-write/struct-read-write/enum-unit-match 样例，含 `println(i32/bool)`、string literal 直接 `println`、string 局部变量 / 参数 / 返回值、`string_len` / `len(string)`、字符串 `==` / `!=`、`to_string(i32/bool/string)`、`string + string`、固定长度数组索引读取 / 元素写入与 `len(array)`、非泛型 struct literal / 参数 by value / 返回值 / 字段读取 / 字段写入，以及非泛型 unit enum tag 值 / 参数 / 返回值 / `== !=` / 语句形态 `match`，有 clang 时可对比解释器和 AOT exe 的 exit code / stdout / stderr |
