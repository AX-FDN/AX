...
| build | AOT/native backend 正处在 LLVM v0 能力包扩展阶段；`build-manifest.json` schema v9 与 `context evidence` 已暴露结构化 `aot_readiness` 和 blocker-level AI 建议，`axc build --json` 会打印同一个 manifest 对象，并且 `axc build` 可为单文件 MIR 子集生成 `generated/main.ll` LLVM IR v0；当前 AOT parity 已覆盖 46 个 core/control-flow/consts/for-in-readonly-slice/runtime-string-slice-for-in/slice-range-read/slice-range-for-in/stdout/string/string-runtime/string-predicate/string-replace/string-split-lines/string-trim/string-pattern/array-read-write-format/struct-read-write-format/struct-pattern/enum-unit/payload-enum/payload-enum-equality/enum-formatter/enum-print/concrete-generic-enum-print/expression-match/range-pattern/or-pattern/match-guard/concrete-Result-Option/result-try 样例，含 top-level `const` v0、`%` 余数运算、`for`、`for in` over fixed array/slice range/runtime string slice、`break`、`continue`、`i32` inclusive range pattern、无绑定 or pattern、guarded match arm、string literal pattern、payload-aware equality、enum formatter、direct enum print、concrete generic enum formatter/print、struct pattern、`println(i32/bool/enum/array/struct)`、string literal 直接 `println`、string 局部变量 / 参数 / 返回值、`string_len` / `len(string)`、字符串 `==` / `!=`、`to_string(i32/bool/string/array/struct/enum)`、`string + string`、`string_contains` / `string_starts_with` / `string_ends_with` 字符串谓词、`string_replace` 全量替换 v0、`string_split_lines` LF/CRLF 行切分并返回只读 `[string]` slice v0、`string_trim` ASCII whitespace v0、固定长度数组索引读取 / 元素写入 / formatter 与 `len(array)`、只读 slice v0 的 `values[start:end]` / `len(slice)` / `slice[index]` / 同文件 slice 参数调用 / slice range 遍历 / runtime `[string]` slice 遍历、非泛型 struct literal / 参数 by value / 返回值 / 字段读取 / 字段写入 / formatter、非泛型 unit enum tag 值 / 参数 / 返回值 / `== !=` / 语句形态 `match`、payload enum constructor / payload read / payload match、表达式形态 `match` / binding pattern / block-valued arm、具体 `Option<i32>` / `Result<i32,string>` enum 实例的 constructor / match / return，以及 `Result<T,E>` 的 `?` Ok 解包 / Err early return v0，有 clang 时可对比解释器和 AOT exe 的 exit code / stdout / stderr |
