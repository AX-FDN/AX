param(
    [string] $OutputDir = "target\json-runtime-smoke"
)
...
fn main() -> i32 {
    let object: string = std.json.object3(
        std.json.field_string("service", "ax"),
        std.json.field_i32("score", 9),
        std.json.field_bool("ok", true)
    );
    println(object);
    println(std.json.array3(std.json.string_value("a"), std.json.i32_value(2), std.json.null_value()));
    println(std.json.string_value("line\nquote\"tab\t"));
    return string_len(object);
}
'@
...
Assert-Equal -Label "axc run exit code" -Actual $LASTEXITCODE -Expected 36

$actualOutput = @($runOutput | ForEach-Object { [string] $_ })
$expectedOutput = @(
    "{""service"":""ax"",""score"":9,""ok"":true}",
    "[""a"",2,null]",
    """line\nquote\""tab\t"""
)
...
if (-not $features.Contains("string_runtime")) {
    Write-Error "AOT readiness did not report string_runtime for std.json."
}

Write-Host "JSON runtime smoke passed at $outputRoot"
