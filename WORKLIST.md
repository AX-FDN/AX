...
- `P0` 仍然优先，因为它是当前地基阻塞
- `P2` 是当前主增长线，因为它直接决定 AX 作为语言是否能继续收口到成熟工具语言
- `P1` 和 `P2` 可以交错推进，但 `P1` 的职责是给语言主线补护城河，不是反过来挤掉 `P2`
- `P3` 只在 `P1/P2` 的阶段产物足够稳定后才激活

当前优先级顺序固定为：

1. 先收掉 `P0` 本机验证路径与外部契约口径
2. 再固定 `P2` 的主代表样例、宿主边界样例和语法优先级顺序
3. 再启动一项最值钱的 `P2` 语法缺口实现，并同步回写样例、回归和文档
4. 再把 `P1` 的 context -> repair/benchmark 输入位接起来
5. 再把 `P1` 的 benchmark 展示页做成公开可引用资产
6. 再收紧 `foundation/`，为 `P3` 标准库冻结准备接口映射

如果 `P1` 和 `P2` 临时抢资源，默认先保证 `P2` 的语言内核推进，再补 `P1` 的展示层和 compare 层。
...
- [ ] 至少一条正式支持的 Windows 本地构建/测试路径写清楚
- [ ] README / quickstart / scripts 对本机验证路径的说法一致
...
- [ ] `P2` 语法优先级顺序已经冻结，不再一边补样例一边随意跳语法点
- [ ] 至少一项 `P2` 语法缺口完成 scope freeze，并进入主线闭环准备

### `P3` 当前只做前置，不做全面启动

- [ ] 第一版 `std.*` 命名空间边界先写清
- [ ] `foundation/* -> std.*` 的映射先列清单
- [ ] 至少有一组样例能作为标准库迁移试点

## 明确阻塞

- [!] `B-01` `[P0]` 当前本机默认 `cargo test` 仍受 `link.exe` 缺失影响
  - 影响：
    - 本机默认 MSVC 路径不可当成“正式可复跑路径”
    - 文档必须明确 GNU 方案或其他正式替代路径
  - 解除标准：
    - 至少一条本机可复跑路径被文档和脚本一起承认
...
- [~] `W-P0-01` 固定 Windows 本地正式可复跑路径
  - 目标：把“CI 可跑但默认本机 `cargo test` 会卡 `link.exe`”收口成明确支持路径，而不是继续模糊带过。
  - 依赖：无。
  - 产物：
    - 一条正式支持的 Windows 本地验证路径
    - 文档中的统一命令口径
  - 完成标准：
    - 至少一条正式支持的 Windows 构建/测试路径写进文档
    - `cargo-gnu.ps1`、quickstart 和 README 口径一致
    - 不再让“本机如何复跑”成为隐性知识

- [ ] `W-P0-02` 写清本机路径与 CI 路径矩阵
...
- [ ] `W-P1-04` 完成一版可公开引用的 benchmark 展示页
...
- [ ] `W-P2-S05` 选择并启动第一项 `P2` 语法实现
  - 目标：在 `match` 第二刀和 payload enum 深化之间，或更小高价值缺口之间，明确先启动哪一项。
...
- [ ] `W-P3-01` 起草 `foundation -> std.*` 的最小迁移清单
...
- [ ] `W-P3-05` 写清宿主 Rust builtin 与 AX 标准接口的边界
