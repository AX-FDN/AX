...
当前 `build` 已经进入 LLVM AOT v0：它稳定导出 source / HIR / MIR / manifest，并能为持续扩展的 MIR 子集生成 `generated/main.ll`，当前已经覆盖单文件、多文件、project-backed 与本地 path package 的第一批 native parity；通过 `axc build <target> --emit exe` 或兼容环境变量 `AX_LLVM_AOT_LINK=1` 显式开启链接并提供 clang 时，已经可以生成 native executable，并用 parity smoke 对比解释器和 AOT exe 的 `exit code / stdout / stderr`。这不是单纯的 IR 展示，而是 AX 解释器语义和 native 输出开始互相校验的编译器路径。后续会按 `LLVM IR 子集 -> AOT runtime ABI -> 包接口 -> 后端 worker 能力 -> 更完整服务端生态` 的顺序持续扩大覆盖面。

**当前 AOT 快照**：LLVM AOT v0 已经进入“按能力包扩张”的阶段。默认 parity smoke 覆盖 **123 个**样例，其中包含 **97 个**单文件/直接样例和 **26 个** `AX.toml` project 样例；仓库内全部 26 个 project 示例都已经进入默认 AOT parity 清单。脚本会完整执行 `check -> run -> build --json -> native exe`，并逐项比较解释器与 native executable 的退出码、标准输出、标准错误。最新补齐的能力包括 **f32 core v0**、固定数组 / struct / slice equality、显式零长度数组 `[]` lowering、`argv_len()` / `argv_get(...)` native CLI 参数读取、`env_has/env_get` host environment ABI v0、`fs/path/process` host ABI v0、`std.env.try_get` project-backed AOT parity、`string_list_new/push/get/join` 与 `len(string_list)` native runtime ABI v0、`std.collections` 源码级 wrapper AOT parity、mutable slice element assignment、短路 `&& / ||` native 语义、AOT 除零与 `i32` overflow 崩溃防护、AOT runtime error 最小 stderr ABI、project-backed AOT linking、本地 path package AOT、project-backed Result/generic 推断，以及 `Result.ok(...)` / `Result.err(...)` 这类泛型静态构造器的期望类型推断。AX 的 AOT 路径已经不只是能算数、能控制流、能返回值，而是在持续把结构化数据、文本输出、宿主边界、相等性语义、项目组织和错误分层一起推向 native 编译闭环。
...
| 编译器 / AOT 版本 | `axc build <file-or-project>` | 正在快速扩展的 LLVM native 编译路径 | 始终导出 `source.ax`、HIR、MIR、`build-manifest.json`；对当前 `i32/bool/f32/string/consts/modulo/for/for-in/slice/string-runtime/argv/env/fs/path/process/array/struct/enum/match/concrete-Result-Option/result-try/project-backed/local-path-package` 支持范围生成 LLVM IR；`--emit exe` / `--emit all` 且有 clang 时可链接 exe |
...
当前 AOT parity smoke 默认覆盖 123 个样例，完整清单由 [`scripts/smoke-aot-parity.ps1`](./scripts/smoke-aot-parity.ps1) 维护。这组样例会依次跑 `check -> run -> build --json -> native exe`，并比较解释器和 exe 的退出码、标准输出、标准错误；其中 26 个是 `AX.toml` project 样例，覆盖单入口、多文件、`module/import`、`std.option/std.result`、`std.collections`、`std.env`、`std.fs`、`std.path`、`std.process`、真实工具型 project 和本地 path package project。新增收口样例包括 [`examples/aot_path_runtime.ax`](./examples/aot_path_runtime.ax)、[`examples/aot_path_join_separator.ax`](./examples/aot_path_join_separator.ax)、[`examples/aot_process_runtime.ax`](./examples/aot_process_runtime.ax)、[`examples/project_config_validate/`](./examples/project_config_validate/)、[`examples/project_package_config/`](./examples/project_package_config/)、[`examples/project_job_runner/`](./examples/project_job_runner/)、[`examples/project_result_pipeline/`](./examples/project_result_pipeline/)、[`examples/project_command_capture/`](./examples/project_command_capture/)、[`examples/project_workspace_audit/`](./examples/project_workspace_audit/) 和 [`examples/project_fs_write_core/`](./examples/project_fs_write_core/) 等。
...
| AX 0.1 Alpha 收口边界       | [`docs/release-0.1-alpha.md`](./docs/release-0.1-alpha.md)         |
| 参与贡献和验证方式          | [`CONTRIBUTING.md`](./CONTRIBUTING.md)                             |
...
| AOT 编译         | LLVM AOT v0 已能为 123 个 core/control-flow/consts/f32-core/host-runtime/string/list/array/slice/struct/enum/match/concrete-Result-Option/result-static-constructors/result-try/project-backed 样例生成 IR、链接 native exe，并与解释器比较 `exit code / stdout / stderr`；其中 26 个是 `AX.toml` project 样例，仓库内全部 project 示例都已进入默认 parity 清单          | [`src/backend/llvm/`](./src/backend/llvm/) [`docs/llvm-aot.md`](./docs/llvm-aot.md)                                  |
