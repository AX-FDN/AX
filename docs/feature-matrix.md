...
| `P4+` AOT / 包接口 / 自举 / 生态 | `[~]` 本地 path package v0 与 `AX.lock` v0 已启动；LLVM AOT v0 已能为持续扩展的单文件 MIR 子集生成文本 IR，并已有 49 个 run vs executable parity 样例；registry、自举和生态仍按 `执行路线.md` 后续阶段推进 |
...
| AOT / JIT | `[~]` | AOT readiness v3 已进入 `build-manifest.json` schema v9 与 `context evidence`；blocker 已带 AI 建议对象；LLVM AOT v0 可为 `fn main() -> i32` 级别子集生成文本 IR，支持 `i32/bool/string` core 子集、top-level `const` v0、`%` 余数运算、`for`、`for in` over fixed array/slice range/runtime `[string]` slice、`break`、`continue`、`i32` inclusive range pattern、无绑定 or pattern、guarded match arm、string literal pattern、struct pattern、payload-aware equality、enum formatter/direct print v0、enum fixed-array/struct/slice payload formatter v0、concrete generic enum formatter/print v0、`println(i32/bool/enum/array/slice/struct)` 最小 stdout ABI、只读 string literal 直接 `println`、string 局部变量 / 参数 / 返回值、`string_len` / `len(string)`、字符串 `==` / `!=`、`to_string(i32/bool/string/array/slice/struct/enum)`、`string + string`、`string_contains` / `string_starts_with` / `string_ends_with` 字符串谓词、`string_replace` 全量替换 v0、`string_split_lines` LF/CRLF 行切分并返回只读 `[string]` slice v0、`string_trim` ASCII whitespace v0、固定长度数组 literal / 参数 by value / 索引读取 / 元素写入 / `len(array)` / formatter、只读 slice v0 的 array-to-slice borrow / `values[start:end]` / `len(slice)` / `slice[index]` / formatter / 同文件 slice 参数调用 / slice range 遍历 / runtime `[string]` slice 遍历、非泛型 struct literal / 参数 by value / 返回值 / field read / field write / formatter、非泛型 unit enum tag 值 / 参数 / 返回值 / `== !=`、语句形态 unit enum `match` 测试、payload enum constructor / read / statement match、表达式形态 `match` / binding pattern / block-valued arm、具体 `Option<i32>` / `Result<i32,string>` enum 实例 constructor / match / return、`Result<T,E>` 的 `?` Ok 解包 / Err early return v0，链接 exe 需显式设置 `AX_LLVM_AOT_LINK=1`；已有 run vs AOT exe parity smoke 覆盖 49 个 core/control-flow/consts/for-in-readonly-slice/runtime-string-slice-for-in/slice-range-read/slice-range-for-in/slice-formatter/stdout/string/string-runtime/string-predicate/string-replace/string-split-lines/string-trim/string-pattern/array-read-write-format/struct-read-write-format/struct-pattern/enum-unit/payload-enum/payload-enum-equality/enum-formatter/enum-print/enum-complex-payload-formatter/concrete-generic-enum-print/expression-match/range-pattern/or-pattern/match-guard/concrete-Result-Option/result-try 样例 | 仍不是发布级 native executable output；JIT 仍不启动；复杂 runtime ABI、项目链接仍未 AOT；string/payload 分配当前是 process-lifetime v0，暂不回收；slice 写入、除 `string_split_lines` 外的 host/runtime slice 来源、跨项目 slice ABI、std project linking、full generics、partial/nested struct destructuring、带绑定 or、复杂 payload equality 等复杂能力仍未进入完整 native layout/lowering | `src/build.rs` `src/backend/llvm/` `src/context.rs` `docs/llvm-aot.md` |
