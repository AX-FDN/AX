...
| `P4+` AOT / 包接口 / 自举 / 生态 | `[~]` 本地 path package v0 与 `AX.lock` v0 已启动；LLVM AOT v0 已能为持续扩展的 MIR 子集生成文本 IR，并已有 123 个 run vs executable parity 样例，其中包含 26 个 `AX.toml` project 样例，覆盖单入口、多文件、`module/import`、`std.option/std.result`、`std.collections`、`std.env`、`std.fs`、`std.path`、`std.process` 与 local path package project-backed AOT；registry、自举和生态仍按 `执行路线.md` 后续阶段推进 |
...
| Repair Archaeology | `[~]` | v0 artifact schema、最小导出脚本和固定 smoke 已落地，目标是继续把 replay / score / compare 产物整理成更完整的 case 级 JSON 与 Markdown 报告 |
| Linux core support | `[x]` | Ubuntu 上核心 `build / check / run / fmt` 与核心测试已进入 CI |
| macOS support | `[ ]` | 当前未进入主线承诺 |
| 原生后端 | `[~]` | `build` 已开始为单文件 MIR 子集生成 LLVM IR v0，并以 run vs executable parity 扩展能力包；当前已覆盖 argv/env/fs/path/process 等第一批 host ABI，但还不是成熟 native backend |
| AOT 后端地基 | `[~]` | Native ABI v1、symbol/mangling 入口、runtime 分区、monomorphization plan 与 native linking plan 已建立；当前保持 CLI、manifest schema 与历史 IR 符号兼容，后续 process/package/std/generics native 支持都沿这组入口扩展 |
| 自举 | `[ ]` | 长期方向，不是当前主线 |
...
| 修复证据展示层 | `[~]` | `Repair Archaeology v0` 已有 artifact schema、最小导出脚本和固定 smoke | 当前还不是 live repair、不是模型客户端、不是新 CLI 契约；更完整 case 级报告仍在继续收口 | `docs/repair-archaeology.md` |
...
| AOT / JIT | `[~]` | AOT readiness v3 已进入 `build-manifest.json` schema version `10` 与 `context evidence`；blocker 已带 AI 建议对象；LLVM AOT v0 可为持续扩展的 MIR 子集生成文本 IR，并在有 clang 时链接 native executable。当前支持 `i32/bool/f32/string` core、top-level `const`、控制流、`for/for in`、runtime error guard、stdout、string runtime、string_list、argv/env/fs/path/process host ABI v0、固定数组、slice、struct、unit/payload enum、payload-aware equality、enum formatter/direct print、表达式 `match`、range/or/guard pattern、具体 `Option/Result`、`Result.ok/err` 推断、`Result ?` early return、project-backed AOT 和 local path package AOT；默认 run vs AOT exe parity smoke 覆盖 123 个样例，其中 26 个是 `AX.toml` project 样例，仓库内全部 project 示例已进入默认清单 | 仍不是发布级 native backend；JIT 仍不启动；string/string_list/payload 分配当前是 process-lifetime v0，暂不回收；除 `string_split_lines` 外的更广泛 host/runtime slice 来源、跨项目 slice ABI、完整 ownership/lifetime、完整 std/native package linking、full generics、partial/nested struct destructuring、带绑定 or、更深层组合 payload equality、registry package native linking 和 host extension ABI 仍未冻结 | `src/build.rs` `src/backend/llvm/` `src/context.rs` `docs/llvm-aot.md` |
