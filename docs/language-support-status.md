...
| Native AOT executable output | planned | planned | planned | planned | partial | partial | partial | LLVM IR v0 can emit `generated/main.ll` for the current AOT subset, including single-entry, multi-source, module/import, std.option/std.result, std.collections, std.env, and pure local path package `AX.toml` projects; executable linking is opt-in with `AX_LLVM_AOT_LINK=1` and depends on clang. The parity smoke now covers 98 cases and compares exit code, stdout, and stderr against `axc run`, including project-backed AOT, i32 overflow/runtime-error guards, f32 core arithmetic/comparison/formatting, string predicates, `string_replace`, `string_split_lines`, `string_trim`, `string_list_new/push/get/join` plus `len(string_list)`, `std.collections` source wrappers, `env_has/env_get` and `std.env.try_get`, slice range `for in`, runtime `[string]` slice `for in`, `argv_len/argv_get` native CLI args, slice reads/writes, slice formatter/direct printing/equality, array formatter/direct printing/equality, explicit zero-length array literals, struct formatter/direct printing/equality, enum formatter v0, direct enum printing, enum fixed-array/struct/slice payload formatting, enum fixed-array/slice payload equality, concrete generic enum formatter/print, generic functions/impls/trait-bound static dispatch, and Result static constructors/try. |
