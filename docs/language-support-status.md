...
| Native AOT executable output | planned | planned | planned | planned | partial | partial | partial | LLVM IR v0 can emit `generated/main.ll` for the current single-file AOT subset; executable linking is opt-in with `AX_LLVM_AOT_LINK=1` and depends on clang. The parity smoke now covers 71 cases and compares exit code, stdout, and stderr against `axc run`, including f32 core arithmetic/comparison/formatting, string predicates, `string_replace`, `string_split_lines`, slice range `for in`, runtime `[string]` slice `for in`, `string_trim`, `argv_len/argv_get` native CLI args, slice reads/writes, slice formatter/direct printing/equality, array formatter/direct printing/equality, explicit zero-length array literals, struct formatter/direct printing/equality, enum formatter v0, direct enum printing, enum fixed-array/struct/slice payload formatting, enum fixed-array/slice payload equality, concrete generic enum formatter/print, generic functions/impls/trait-bound static dispatch, and Result static constructors/try. |
