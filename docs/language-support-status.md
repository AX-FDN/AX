...
| Arrays and read-only slices | stable | stable | stable | stable | stable | stable | partial | Fixed array read/write, `to_string(array)`, direct `println(array)`, and read-only slice v0 are in LLVM AOT v0, including fixed-array iteration, `values[start:end]`, direct iteration over slice ranges, `len(slice)`, `slice[index]`, same-file slice parameter calls, and `string_split_lines` runtime `[string]` slice iteration. Slice writes, host/runtime-produced slices beyond `string_split_lines`, and cross-project slice ABI remain pending. Dynamic collections are still represented through dedicated std helpers. |
...
| Native AOT executable output | planned | planned | planned | planned | partial | partial | partial | LLVM IR v0 can emit `generated/main.ll` for the current single-file AOT subset; executable linking is opt-in with `AX_LLVM_AOT_LINK=1` and depends on clang. The parity smoke now covers 46 cases and compares exit code, stdout, and stderr against `axc run`, including string predicates, `string_replace`, `string_split_lines`, slice range `for in`, runtime `[string]` slice `for in`, `string_trim`, slice range reads, array formatter/direct printing, struct formatter/direct printing, enum formatter v0, direct enum printing, and concrete generic enum formatter/print. |
