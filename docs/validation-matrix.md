...
这条脚本会对 71 个 AOT 样例比较解释器和 native executable 的 `exit code / stdout / stderr`，完整清单由 `scripts/smoke-aot-parity.ps1` 维护。当前覆盖 core/control-flow/consts/f32-core/stdout/string/string-runtime/string-predicate/string-replace/string-split-lines/string-trim/string-pattern/argv/fixed-array-read-write-format-equality/zero-length-array/for-in-readonly-slice/runtime-string-slice-for-in/slice-range-read/slice-range-for-in/slice-formatter-equality/struct-read-write-format-equality/struct-pattern/enum-unit/payload-enum/payload-enum-equality/enum-formatter/enum-print/enum-complex-payload-formatter/enum-array-slice-payload-equality/concrete-generic-enum-print/expression-match/range-pattern/or-pattern/match-guard/concrete-Result-Option/result-try。其中包含 top-level `const` v0、`%` 余数运算、`f32` 算术/比较/`println`/`to_string`、`for`、`for in` over fixed array/slice range/runtime `[string]` slice、`break`、`continue`、`i32` inclusive range pattern、无绑定 or pattern、guarded match arm、string literal pattern、payload-aware equality、enum formatter、direct enum print、enum 固定数组/struct/slice payload formatter、concrete generic enum formatter/print、struct pattern、`println(i32/bool/f32/enum/array/slice/struct)`、string literal 直接 `println`、string 局部变量 / 参数 / 返回值、`string_len` / `len(string)`、字符串 `==` / `!=`、`to_string(i32/bool/f32/string/array/slice/struct/enum)`、`string + string`、`string_contains` / `string_starts_with` / `string_ends_with` 字符串谓词、`string_replace` 全量替换 v0、`string_split_lines` LF/CRLF 行切分并返回只读 `[string]` slice v0、`string_trim` ASCII whitespace v0、`argv_len/argv_get` native CLI args、固定长度数组索引读取 / 元素写入 / formatter / equality 与 `len(array)`、只读 slice v0 的 `values[start:end]` / `len(slice)` / `slice[index]` / formatter / equality / 同文件 slice 参数调用 / slice range 遍历 / runtime `[string]` slice 遍历、非泛型 struct literal / 参数 by value / 返回值 / 字段读取 / 字段写入 / formatter / equality、非泛型 unit enum tag 值 / 参数 / 返回值 / `== !=` / 语句形态 `match`、payload enum constructor / read / match / fixed-array payload equality / slice payload equality、表达式形态 `match` / binding pattern / block-valued arm、具体 `Option<i32>` / `Result<i32,string>` enum 实例 constructor / match / return，以及 `Result<T,E>` 的 `?` Ok 解包 / Err early return parity。如果只想验证单个 executable 链接与退出码，仍可跑 `.\scripts\smoke-aot-link.ps1`。没有 clang 的本机不要求这两条通过；必须保证默认 IR-only 路径和缺 clang 的 `AOT1001` blocker 路径稳定。
