...
这条脚本会对 36 个 core/control-flow/consts/for-in-readonly-slice/slice-range-read/stdout/string/string-predicate/string-pattern/array-read-write/struct-read-write/struct-pattern/enum-unit/payload-enum/payload-enum-equality/expression-match/range-pattern/or-pattern/match-guard/concrete-Result-Option/result-try AOT 样例比较解释器和 native executable 的 `exit code / stdout / stderr`，其中包含 top-level `const` v0、`%` 余数运算、`for`、`for in` over fixed array、`break`、`continue`、`i32` inclusive range pattern、无绑定 or pattern、guarded match arm、string literal pattern、payload-aware equality、struct pattern、`println(i32/bool)`、string literal 直接 `println`、string 局部变量 / 参数 / 返回值、`string_len` / `len(string)`、字符串 `==` / `!=`、`to_string(i32/bool/string)`、`string + string`、`string_contains` / `string_starts_with` / `string_ends_with` 字符串谓词、固定长度数组索引读取 / 元素写入与 `len(array)`、只读 slice v0 的 `values[start:end]` / `len(slice)` / `slice[index]` / 同文件 slice 参数调用、非泛型 struct literal / 参数 by value / 返回值 / 字段读取 / 字段写入、非泛型 unit enum tag 值 / 参数 / 返回值 / `== !=` / 语句形态 `match`、payload enum constructor / read / match、表达式形态 `match` / binding pattern / block-valued arm、具体 `Option<i32>` / `Result<i32,string>` enum 实例 constructor / match / return，以及 `Result<T,E>` 的 `?` Ok 解包 / Err early return parity。如果只想验证单个 executable 链接与退出码，仍可跑 `.\scripts\smoke-aot-link.ps1`。没有 clang 的本机不要求这两条通过；必须保证默认 IR-only 路径和缺 clang 的 `AOT1001` blocker 路径稳定。
