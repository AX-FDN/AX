...
| `std.process` | 命令执行、工作目录内执行、输出捕获、状态型 Result 安全接口和 `ProcessStatus` | `std/process.ax` 与宿主 `process_*` builtin | 保持小而明确，不引入 async 或流式进程 API |
| `std.report` | key/value 行、section、path stat、bool/int/string stat | `std/report.ax` 与 `foundation/report.ax` | 只放确定性文本报告构造，不放展示主题系统 |
| `std.result` | 显式可能失败值：`Result<T,E>`、`Ok(T)`、`Err(E)`、基础查询、fallback 与 `?` 消费形态 | `std/result.ax` | 作为官方失败返回约定，支持最小 `?` 错误传播，不引入异常系统 |
| `std.workspace` | 工作区条目显示、深度前缀、递归扫描辅助 | `std/workspace.ax`、`foundation/workspace.ax` 与部分样例私有逻辑 | 只承接小型工具常用 workspace 输出，不做完整项目索引器 |
| `std.collections` | `string_list` 官方源码级包装 | `std/collections.ax` 与宿主 `string_list_*` builtin | 第一版只冻结已有最小集合 helper，不提前上泛型 collections |
...
| [`../std/path.ax`](../std/path.ax) | `std.path` | `join / parent / file_name / stem / extension / resolve / classify_file_kind / is_text_file` | 第一、第二、第三、第五、第十试点已用 |
| [`../std/process.ax`](../std/process.ax) | `std.process` | 命令执行、工作目录内执行、输出捕获、`ProcessStatus` 与状态型 `try_run / try_run_in / try_status / try_status_in` | 第四、第五、第九、第十试点已用 |
| [`../std/report.ax`](../std/report.ax) | `std.report` | key/value 报告、路径报告、section 片段 | 第一、第二、第三、第四、第五、第十试点已用 |
| [`../std/result.ax`](../std/result.ax) | `std.result` | `Result<T,E>`、`Ok(T)`、`Err(E)`、静态构造 `Result.ok / Result.err`、`is_ok / is_err / unwrap_or / error_or / prefix_error / replace_error` | `project_option_result` 与第七到第十一 Result 试点已用 |
...
| `std.process` | `ProcessStatus / status_from_code / run / try_run / try_status / run_in / try_run_in / try_status_in / capture_in` | command capture、command batch、process/result smoke、result pipeline、job runner | 冻结同步命令执行、状态型 Result 接口、基础状态结构与输出捕获；不冻结 `try_capture_in`、stdout/stderr 结构体、shell contract、async 或 streaming |
| `std.report` | `append_line / append_string_stat / append_int_stat / append_bool_stat / append_path_stat / begin_section / append_section_details_or_none` | 多组迁移试点、result pipeline、job runner | 只冻结确定性 key/value 与 section 文本报告，不做表格、主题、颜色或富文本 |
| `std.result` | `Result<T,E> / Ok(T) / Err(E) / Result.ok / Result.err / is_ok / is_err / unwrap_or / error_or / prefix_error / replace_error / expr?` | option/result、env/result、file/result、process/result、result propagation、result pipeline、config validate smoke、job runner | 冻结显式失败值约定、最小错误传播语法和 string 错误标注 helper；结构化错误类型继续由项目级 enum 先试点 |
...
| `std.process` | command capture、command batch、process/result smoke、result pipeline、job runner | 两组命令类运行夹具覆盖 `capture_in / run / run_in`；`project_process_result_runs_on_controlled_fixture`、`project_result_pipeline_runs_on_controlled_fixture` 与 `project_job_runner_runs_on_controlled_fixture` 覆盖 `ProcessStatus / try_run / try_run_in / try_status_in`；build source tree 确认 `std/process.ax` 被复制 |
| `std.report` | 多组迁移试点、result pipeline、job runner | 多组运行夹具覆盖 deterministic 文本报告构造；build source tree 确认 `std/report.ax` 被复制 |
| `std.result` | option/result smoke、env/result smoke、file/result smoke、process/result smoke、result pipeline、config validate、job runner | `project_option_result_runs` 覆盖 `Result.ok / Result.err / is_ok / unwrap_or / error_or`；`project_env_result_runs`、`project_file_result_runs_on_controlled_fixture`、`project_process_result_runs_on_controlled_fixture`、`project_result_pipeline_runs_on_controlled_fixture` 与 `project_job_runner_runs_on_controlled_fixture` 覆盖宿主边界 Result 返回消费；`project_config_validate_runs_on_controlled_fixture` 覆盖项目级 error enum 转换后继续用 `?` 传播；build source tree 确认 `std/result.ax` 被复制 |
...
- 第一版 `std.env` 从 `has / get` 起步，后续补入 `try_get` 作为 Result 风格安全读取；仍不提前设计默认值。
...
- 为标准库失败返回值、`?` 错误传播和 repair contract 统一返回形态提供前置接口。
...
- 本轮仍不引入默认值参数、异常系统或结构化错误类型；`?` 已可消费 `try_get` 返回的 `Result`。
...
- interface snapshots 已覆盖该项目的 `check / run / build source tree`。
- 本轮仍不引入异常系统、权限模型、流式 IO 或写侧 Result 接口；`?` 已可消费读侧 `try_*` 返回的 `Result`。

第九迁移试点：[`../examples/project_process_result/`](../examples/project_process_result/)

理由：

- 把 `std.result.Result<T,E>` 推进到进程状态边界，覆盖空命令和缺失工作目录这两个可前置判断的失败点。
- `process_run / process_run_in` 本来返回退出码，适合先升级成 `Result<i32,string>` 的状态型接口。
- `process_capture_in` 的非零退出当前仍是 runtime error，AX 还不能捕获，因此本轮不提供 `try_capture_in`。

当前迁移结果：

- `std.process` 新增 `ProcessStatus / status_from_code / try_run / try_run_in / try_status / try_status_in`。
- `AX.toml` 使用 `sources = ["../../std"]`。
- `src/main.ax` 通过 `std.process.try_run / try_run_in / try_status_in`、`ProcessStatus` 和 `std.result.unwrap_or / error_or` 消费状态型进程失败返回约定。
- interface snapshots 已覆盖该项目的 `check / run / build source tree`。
- 本轮仍不引入 async、streaming process、结构化 stdout/stderr/exit-code、shell contract 或 `try_capture_in`。

第十迁移/压力试点：[`../examples/project_result_pipeline/`](../examples/project_result_pipeline/)

理由：

- 把 `std.fs.try_read_to_string`、`std.env.try_get`、`std.process.try_status_in` 放进同一个真实小流水线，观察 `Result` 组合错误流里的成本。
- 这个样例不是为了新增标准库 API，而是为了验证 `?` 在真实宿主边界组合中的可读性和回归稳定性。
- 同时验证 `std.cli / std.fs / std.path / std.env / std.process / std.result / std.report` 在一个 project-backed 工具里能稳定组合。

当前迁移结果：

- `AX.toml` 使用 `sources = ["../../std"]`。
- `src/main.ax` 通过 `?` 消费文件读取、环境变量读取和进程状态三个成功路径，并保留显式 `match` 处理最终 pipeline 结果和缺失文件分支。
- 输出稳定的 `RESULT-PIPELINE.txt`，只记录 `env_path_ok=true`，不把本机 `PATH` 内容写入快照。
- interface snapshots 已覆盖该项目的 `check / run / build source tree`。
- 本轮仍不引入异常系统、结构化错误类型、`try_capture_in` 或隐式错误类型转换；`?` 只作为 `Result` 形状的显式提前返回语法。

第十一迁移/压力试点：[`../examples/project_config_validate/`](../examples/project_config_validate/)

理由：

- 把错误传播从统一 `string` 错误推进到项目级 `ConfigError` enum，验证 AX 后端工具常见的分层错误模型。
- 通过 `from_io<T>` 把 `std.fs.try_read_to_string` 的宿主 string 错误显式转换为 `ConfigError.Io`，再用 `?` 在同一函数内传播。
- 同时验证 `std.cli / std.fs / std.path / std.report / std.result` 可以承载配置校验、报告输出和失败路径。

当前迁移结果：

- `AX.toml` 使用 `sources = ["../../std"]`。
- `src/main.ax` 定义 `ConfigError.Io / MissingField / InvalidField`，成功路径生成 `CONFIG-VALIDATION.txt`，失败路径输出稳定 `config_error=...`。
- interface snapshots 已覆盖该项目的 `check / run / build source tree`。
- 当前边界：项目级 error enum 仍靠显式转换 helper，不做隐式 `From`、异常系统、try/catch 或错误 trait。

第十二迁移/压力试点：[`../examples/project_collections_report/`](../examples/project_collections_report/)
...
- `AX.toml` 使用 `sources = ["../../std"]`。
- `src/main.ax` 通过 `std.collections` 组装标签列表，生成 `COLLECTIONS-REPORT.txt`。
- interface snapshots 已覆盖该项目的 `check / run / build source tree`。
- 当前边界：只冻结 `string_list` 包装，不做泛型 list/map/set、iterator、排序、高阶函数或集合 trait。

第十三迁移/压力试点：[`../examples/project_job_runner/`](../examples/project_job_runner/)

理由：

- 把本地 path package v0 从配置校验样例推进到 worker/job runner 风格负载，验证 `AX.lock`、包图 readiness、项目内依赖包源码和主项目入口可以在同一条链路中稳定组合。
- 同时覆盖 `std.cli / std.env / std.fs / std.path / std.process / std.report / std.result`，让标准库候选在更接近后端 worker 的场景里承受压力。
- 该样例仍然坚持 AX 包接口优先：用户导入的是 `job_rules.*` AX 包模块，不暴露 Rust crate、Cargo dependency 或宿主实现细节。

当前迁移结果：

- 根项目通过 `[dependencies] job_rules = { path = "packages/job_rules" }` 引入本地 AX 包。
