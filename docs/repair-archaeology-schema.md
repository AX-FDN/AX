...
1. 先写 `export-repair-archaeology.ps1`，只读取现有 artifact。
2. 先支持 `base -> ai` comparison，再扩到 `cold -> base -> ai`。
3. 先输出 JSON，再从 JSON 渲染 Markdown。
4. 至少覆盖一个 improved case、一个 both-pass case、一个 failed/regressed case。
5. 再把 smoke 回归接入 interface snapshots 或脚本 smoke。
