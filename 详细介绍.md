...
1. 只使用当前仓库已实现的原型语法：`match`、模块/import、methods/impl、泛型结构体/函数与 traits/interfaces 第一刀已经可用；不要发明 trait bounds、动态派发、异常、async、where 约束或泛型方法。空数组字面量 `[]` 只允许出现在显式零长度数组上下文里，例如 `let values: [i32; 0] = [];`。
