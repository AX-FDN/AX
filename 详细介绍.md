...
- 解释器：`main`、局部变量、函数调用、递归、算术/比较、条件、循环、内置 `println` / `string_len`、结构体、固定长度数组、只读切片、枚举值
...
- 字符串当前支持 `string + string` 拼接，以及内置 `string_len(text)` 长度查询
...
- 想直接照着例子练：看 [`examples/hello.ax`](./examples/hello.ax)、[`examples/arrays.ax`](./examples/arrays.ax)、[`examples/factorial.ax`](./examples/factorial.ax)、[`examples/for_loop.ax`](./examples/for_loop.ax)、[`examples/slices.ax`](./examples/slices.ax)、[`examples/string_tools.ax`](./examples/string_tools.ax)、[`examples/syntax_overview.ax`](./examples/syntax_overview.ax)
...
1. 只使用当前仓库已实现的原型语法，不要发明 `match`、空数组字面量、模块、泛型、异常、async。
...
8. 当前内置函数是 `println(...)` 和 `string_len(text)`。
