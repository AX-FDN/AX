...
- 想直接照着例子练：看 [`examples/hello.ax`](./examples/hello.ax)、[`examples/arrays.ax`](./examples/arrays.ax)、[`examples/factorial.ax`](./examples/factorial.ax)、[`examples/for_loop.ax`](./examples/for_loop.ax)、[`examples/slices.ax`](./examples/slices.ax)、[`examples/string_tools.ax`](./examples/string_tools.ax)、[`examples/traversal.ax`](./examples/traversal.ax)、[`examples/format_report.ax`](./examples/format_report.ax)、[`examples/syntax_overview.ax`](./examples/syntax_overview.ax)
...
8. 当前内置函数是 `println(...)`、`string_len(text)`、`len(value)` 和 `to_string(value)`；其中 `len` 可用于 `string`、固定长度数组与只读切片，`to_string` 可把具体运行时值转成 `string` 以便做最小报告拼接。
