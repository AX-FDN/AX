...
- `axc build <path>`：导出 build manifest、HIR/MIR 和源码副本等骨架产物，供后续原生后端阶段使用

当前最小可运行子集已经支持：

- 顶层：`fn`、`struct`、`enum`
- 语句：`let`、`let mut`、变量赋值、结构体字段赋值、表达式语句、`return`、`if / else`、`while`、`for`
- 表达式：整数、浮点、布尔、字符串、变量引用、一元运算、二元运算、函数调用、结构体字面量、数组字面量、字段访问、索引读取、切片表达式、枚举值引用
- 解释器：`main`、局部变量、函数调用、递归、算术/比较、条件、循环、内置 `println` / `string_len` / `len` / `to_string`、结构体、固定长度数组、只读切片、枚举值
...
- 只读切片类型写作 `[Type]`，切片表达式写作 `values[start:end]`
- 固定长度数组类型写作 `[Type; N]`，字面量写作 `[expr, ...]`，当前支持读取 `values[index]`
- 固定长度数组当前也支持直接元素写入：`values[index] = expr;`，前提是数组变量用 `let mut` 声明
- 字符串当前支持 `string + string` 拼接，以及内置 `string_len(text)`、统一长度查询 `len(value)` 和最小格式化能力 `to_string(value)`
- 可写目标当前支持嵌套路径赋值，例如 `outer.inner.value = expr;` 和 `tokens[index].value = expr;`；只读切片仍然禁止写入
- 循环控制当前已支持 `break;`，可用于提前退出最近一层 `while` 或 `for`
...
cargo run -- check examples\string_tools.ax
...
查看 MIR：
```powershell
cargo run -- mir examples\syntax_overview.ax
```
...
cargo run -- run examples\string_tools.ax
...
需要注意：当前 `axc build` 还不是“生成原生可执行文件”的成熟后端，它现在稳定导出的是 build 骨架产物，例如 `build-manifest.json`、`program.hir.json`、`program.mir.json`、源码副本与计划中的 `bin/` 目录。这是阶段 4 之前的刻意设计，不应误解成 native backend 已经完成。
...
.\scripts\cargo-gnu.ps1 run -- check examples\string_tools.ax
...
.\scripts\cargo-gnu.ps1 run -- run examples\string_tools.ax
...
如果当前 shell 默认拦本地 `.ps1`，先在这个 shell 里执行一次：
...
默认输出会写到 `.ax-ai\diagnostics-benchmark\<timestamp>\`，并至少生成：

- `summary.json`：机器可读的稳定性能报告
- `summary.md`：便于人直接阅读的基线摘要

`summary.json` 现在也会直接给出三组固定的相对开销比较：
...
```powershell
.\scripts\benchmark-diagnostics.ps1 -Iterations 10 -OutputDir .ax-ai\diagnostics-benchmark\manual-run
```
...
下一轮证据链展示层是 [`docs/repair-archaeology.md`](./docs/repair-archaeology.md)。
它的目标不是调用真实模型，也不是新增 `axc generate`，而是把已有 repair replay、score、compare 和 context-enabled export 产物整理成按 case 可查询、可导出的 JSON / Markdown 修复报告。

Repair Archaeology v0 会回答更细的问题：

- 这个 case 的初始诊断是什么
- 哪个 `rule_id / repair_goal` 参与了修复
- `cold / base / ai` 各自是否通过
- 哪一步失败或退化
- context bundle 是否进入输入
- 这条证据如何复现
...
- [`scripts/smoke-benchmark-diagnostics.ps1`](./scripts/smoke-benchmark-diagnostics.ps1) 会固定回归 `benchmark-diagnostics.ps1` 的 `summary.json` 契约
...
- [`.github/workflows/ci.yml`](./.github/workflows/ci.yml) 会在 GitHub Actions 上执行 `cargo fmt --check`、Rust tests、diagnostics benchmark smoke run、repair benchmark smoke run、repair comparison smoke run 和 repair mode comparison smoke run

如果你需要稳定字段说明，可以直接看 [`docs/diagnostics-benchmark-schema.md`](./docs/diagnostics-benchmark-schema.md)。

## 推荐阅读顺序

- 想了解项目边界与阶段路线：看 [`执行路线.md`](./执行路线.md)
- 想按当前仓库真实语法写代码：看 [`SYNTAX.md`](./SYNTAX.md)
- 想直接照着例子练：看 [`examples/hello.ax`](./examples/hello.ax)、[`examples/arrays.ax`](./examples/arrays.ax)、[`examples/factorial.ax`](./examples/factorial.ax)、[`examples/for_loop.ax`](./examples/for_loop.ax)、[`examples/break_loop.ax`](./examples/break_loop.ax)、[`examples/slices.ax`](./examples/slices.ax)、[`examples/string_tools.ax`](./examples/string_tools.ax)、[`examples/traversal.ax`](./examples/traversal.ax)、[`examples/format_report.ax`](./examples/format_report.ax)、[`examples/token_rewrite.ax`](./examples/token_rewrite.ax)、[`examples/syntax_overview.ax`](./examples/syntax_overview.ax)
- 想看更贴近“编译器/工具内部逻辑”的 AX 样例：看 [`examples/bootstrap_token_scan.ax`](./examples/bootstrap_token_scan.ax)、[`examples/bootstrap_state_machine.ax`](./examples/bootstrap_state_machine.ax) 和 [`examples/bootstrap_block_summary.ax`](./examples/bootstrap_block_summary.ax)

## 给 AI 的最小规则

如果你把 README 直接喂给模型，请让它严格遵守下面这些规则：

1. 只使用当前仓库已实现的原型语法：`match`、模块/import、methods/impl、泛型结构体/函数/enum 与 traits/interfaces 第一刀已经可用；不要发明 trait bounds、动态派发、异常、async、where 约束或泛型方法。空数组字面量 `[]` 只允许出现在显式零长度数组上下文里，例如 `let values: [i32; 0] = [];`。
...
7. `for` 使用 `for (init; condition; step) { ... }`；推荐 `for (let mut i: i32 = 0; i < n; i = i + 1) { ... }`，并可在循环体中使用 `break;` 提前退出。
8. 当前内置函数是 `println(...)`、`string_len(text)`、`len(value)` 和 `to_string(value)`；其中 `len` 可用于 `string`、固定长度数组与只读切片，`to_string` 可把具体运行时值转成 `string` 以便做最小报告拼接，嵌套路径赋值则可直接更新结构体内部字段和数组元素中的字段。
9. 当前循环体内可使用 `break;` 提前退出最近一层 `while` 或 `for`；循环外使用会被语义检查拒绝。
