...
- [x] `Q-P5-02` lockfile v0
  ...
  - 已覆盖：CLI 参数解析、temp project lockfile smoke、`--check` 验证、stale package graph drift 明细、context `overview/topology/evidence` 的 `local_package_lock` 状态与 AI-facing issues、`context evidence` 与 `build-manifest.json` 的 `package_graph_readiness`、interface contract、feature matrix、README。
